...
export class AlertMonitorNotFoundError extends Error {
	constructor(alertMonitorId: number) {
		super(`alert monitor ${alertMonitorId} does not exist or is not available to your client`);
...
async function findAccessibleMonitor(
	...
	const canAccessExternal =
		"canWriteExternal" in access ? access.canWriteExternal : access.canReadExternal;
	const monitor = canAccessExternal
		? await queries.findAlertMonitorById(db, alertMonitorId)
		: await queries.findAlertMonitorByIdForClient(db, alertMonitorId, session.client_id);
...
async function ensureAlertExists(db: Kysely<DB>, alertId: number): Promise<void> {
...
export async function listAlertMonitors(
	...
	const [configs, activities, overrides, channelLinks, ranges] = access.canReadExternal
		? await Promise.all([
		...
		: await Promise.all([
				...
				queries.listActivityOverridesForClient(db, session.client_id),
	...
	return monitors.map((monitor) => ({
		...
		configs: configs.filter((row) => row.alert_monitor_id === monitor.id).map(serialize),
		activities: activities.filter((row) => row.alert_monitor_id === monitor.id).map(serialize),
		...
		channel_links: channelLinks
			.filter((row) => row.alert_monitor_id === monitor.id)
			.map(serialize),
		ranges: ranges.filter((row) => row.alert_monitor_id === monitor.id).map(serialize),
	}));
}

function toStatusResponse(row: AlertMonitorStatusRow): AlertMonitorStatusResponse {
	return {
		...
		range:
			row.range_id === null || row.min_value === null || row.max_value === null
				? null
				: {
						id: row.range_id,
		...
		measurement_record_latest:
			row.latest_measurement_record_id === null || row.latest_measurement_record_date === null
				? null
				: {
...
export async function listAlertMonitorStatuses(
	...
	const rows = access.canReadExternal
		...
		: await queries.listAlertMonitorStatusesForClient(db, session.client_id);
...
export async function createChannelLink(
	...
	const monitor = await findAccessibleMonitor(db, input.alert_monitor_id, session, access);
	const channel = access.canWriteExternal
		? await queries.findChannelById(db, input.channel_id)
		: await queries.findChannelByIdForClient(db, input.channel_id, session.client_id);
...
export async function createRange(
	...
	if (input.min_value >= input.max_value) {
		throw new AlertMonitorRelationshipError("min_value must be less than max_value");
	}
