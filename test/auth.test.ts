...
// Open signup is currently allowed — the sign-up guard in
// src/modules/auth/routes.ts is commented out. If invite-only is re-enabled
// there, flip this test back to expecting a 4xx and zero rows.
test("open signup creates a user", async () => {
	...
	const users = await db.pool.query(
		`SELECT client_id, role_id FROM "user" WHERE email = 'walk-in@example.com'`,
	);
	expect(users.rows).toHaveLength(1);
	expect(users.rows[0]).toEqual({ client_id: 1, role_id: 1 });
});
